...
- The [admission webhook](architecture.md#webhooks) covers Akamai storage misconfiguration specifically; everything else still relies on CRD-level CEL rules (`+kubebuilder:validation:XValidation`, covering PDB and autoscaling constraints) rather than webhook validation.
...
- Bucket versioning and lifecycle policy (AWS) are currently hardcoded, not configurable per-`Application` — planned for a future release.
- If an `Application` is deleted and recreated, it gets a new Kubernetes UID, so it can't currently reclaim a bucket it previously owned — see the note in [Ownership verification](authentication-and-storage.md#ownership-verification).
