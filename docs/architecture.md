...
**Defaulting** resolves and writes the effective `spec.storage.secretName` onto the object at admission time for every provider, so `kubectl get -o yaml` always shows the real output Secret name instead of requiring you to know the operator's internal fallback logic (`naming.StorageSecret`). This is safe unconditionally because `spec.storage.secretName` only ever names the operator's own generated output Secret — it's unrelated to input credentials, which live in their own per-provider fields (`spec.storage.aws.credentialsSecretRef`, `spec.storage.akamai.accessKeySecretRef`). When `spec.storage.provider: Akamai`, the defaulter additionally resolves and writes `spec.storage.akamai.accessKeySecretRef`.
...
- an Akamai config where `secretName` (the operator's generated output Secret) collides with `accessKeySecretRef` (your input token Secret) — the operator owns and deletes the former, so this would corrupt or destroy your token Secret; the same check applies to AWS's `secretName` vs `spec.storage.aws.credentialsSecretRef`;
