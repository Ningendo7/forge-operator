...
| `forge_storage_bucket_adopted_total` | Counter | `provider` | Incremented every time the adopt-bucket annotation actually results in a claim — an audit signal, not an incident (see [Explicit adoption](authentication-and-storage.md#explicit-adoption)) |
| `forge_storage_ownership_reclaimed_total` | Counter | `provider` | Incremented every time an `Application`'s stable ownership ID automatically reclaims a bucket after its Kubernetes UID changed (a Velero restore or cluster migration) — distinct from adoption above, since no human authorized it, the same Application just came back (see [Automatic reclaim after a restore](authentication-and-storage.md#automatic-reclaim-after-a-restore)) |
