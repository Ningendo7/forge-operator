...
Unlike credential cleanup, **bucket deletion itself re-verifies ownership before touching anything** — the same tag/marker check the create path uses, but never claimed or adopted here, only checked. If it doesn't match (someone else's ownership tag, or none at all with no durable record of this operator having created or adopted it), cleanup refuses to delete the bucket or its contents and deliberately leaves the finalizer stuck, surfaced as a `Degraded` condition rather than a silent skip — the same "never delete something we don't own" reasoning as the create path, just applied at delete time instead. This is a *deliberate* stuck state, not a bug: resolving it means a human confirms who actually owns the bucket and either fixes the tag/marker or removes the finalizer manually. IAM role/access key cleanup still runs regardless, since it never depends on bucket ownership.
